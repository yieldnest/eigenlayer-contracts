...
interface ISlashableProportionOracle {
    ...
    /**
     * @notice Returns the proportion of the operator's strategy shares that are slashable by a certain AVS at the given epoch
     * @param avs the address of the AVS
     * @param operator the address of the operator
     * @param strategy the address of the strategy
     * @param epoch the epoch at which to check the slashable proportion
     */
    function getSlashableProportionOfStrategyAtEpoch(address avs, address operator, IStrategy strategy, uint32 epoch) external view returns (uint16);
